...
class Reputation(models.Model):
    ...
    giver = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="given_reputations"
    )
    receiver = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="received_reputations"
    )
    ...
    def clean(self):
        ...
        if self.giver.created_at > timezone.now() - timedelta(days=7):
            raise ValidationError(
                "You must be registered for at least 7 days to give reputation."
            )
